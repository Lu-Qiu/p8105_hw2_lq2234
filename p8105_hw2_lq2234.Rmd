...
* The dataset `amyloid_df` is imported without the first row using arguments in `read_csv`. Then, `pivot_longer` is used to switch the dataset from “wide” to “long” format. The column name 'study_id' is changed to 'id' to match with the `baseline_df` dataset. The data type of `amyloid_ratio` is converted to numeric using `mutate`. Please note that in this step, all 'Na' values in this column are converted to `NA` due to coercion.
