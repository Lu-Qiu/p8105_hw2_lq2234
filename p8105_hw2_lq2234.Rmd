...
* The `pols_month` dataset has `r nrow(pols_month)` observations and `r ncol(pols_month)` variables and tells us about the party affiliation distribution (democrat or republican) for governors and senators for a given year from years `r pols_month |> pull(year) |> min()` to `r pols_month |> pull(year) |> max()`. It also tells us whether the sitting president was a democrat or republican. The names of key variables are: `r names(pols_month)`.
* The `snp_df` dataset has `r nrow(snp_df)` observations and `r ncol(snp_df)` variables, ranging from years `r snp_df |> pull(year) |> min()` to `r snp_df |> pull(year) |> max()`. The names of key variables are: `r names(snp_df)`.
* The `unemployment_df` dataset has `r nrow(unemployment_df)` observations and `r ncol(unemployment_df)` variables, ranging from years `r unemployment_df |> pull(year) |> min()` to `r unemployment_df |> pull(year) |> max()`. The names of key variables are: `r names(unemployment_df)`.
...
* The total weight of trash collected by Professor Trash Wheel is `r sum(pull(professor_wheel, weight_tons))` tons. The total number of cigarette butts collected by Gwynnda in July of 2021 is `r gwynnda_wheel |> filter(year == 2021, month == 'July') |> pull(cigarette_butts) |> sum()`.
...
* Relevant features of the dataset: `r nrow(baseline_df)` participants were recruited, and of these `r sum(pull(baseline_df, age_at_onset) != '.')` developed MCI. The average baseline age is `r baseline_df |> pull(current_age) |> mean() |> round(2)`. A proportion of `r ((sum(pull(baseline_df, sex) == 0 & pull(baseline_df, apoe4) ==1)/sum(pull(baseline_df, sex) == 0))*100) |> round(2)` % of women in the study are APOE4 carriers.
